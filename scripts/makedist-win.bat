...
if %1 == 1 (echo Making SpectralHarp Windows DEMO VERSION distribution ...) else (echo Making SpectralHarp Windows FULL VERSION distribution ...)
...
msbuild SpectralHarp.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild SpectralHarp.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SpectralHarp.aaxplugin\Contents\Win32\SpectralHarp.aaxplugin --out .\build-win\aax\bin\SpectralHarp.aaxplugin\Contents\Win32\SpectralHarp.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SpectralHarp.aaxplugin\Contents\x64\SpectralHarp.aaxplugin --out .\build-win\aax\bin\SpectralHarp.aaxplugin\Contents\x64\SpectralHarp.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\SpectralHarp.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\SpectralHarp.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "SpectralHarp Installer" ".\installer\SpectralHarp Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\SpectralHarp Installer.exe" ".\installer\SpectralHarp Demo Installer.exe"
REM -del ".\installer\SpectralHarp Installer.exe"
REM -)
